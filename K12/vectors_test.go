...
func init() {
	...
	vectors = append(vectors, []testVectors{
...
func TestVectors(t *testing.T) {
    ...
    out := make([]byte, vector.outputLen)
    K12Sum(vector.customString, vector.message, out)
		if len(out) == 10032 {
			out = out[10000:]
		}
    if temp := hex.EncodeToString(out); temp != vector.expectedOutput {
			t.Error(vector.message, len(vector.message))
